...
import (
	...
	"github.com/Ofsen/orders-api/model"
...
func (r *RedisRepo) Insert(ctx context.Context, order model.Order) error {
	...
	if err != nil {
		return fmt.Errorf("failed to encore order: %w", err)
	}
...
func (r *RedisRepo) FindAll(ctx context.Context, page FindAllPage) (FindAllResult, error) {
	...
	if err != nil {
		return FindAllResult{}, fmt.Errorf("failed to get order ids: %w", err)
	}
	...
	for i, x := range xs {
		...
		if err != nil {
			return FindAllResult{}, fmt.Errorf("failed to decode order json: %w", err)
		}
